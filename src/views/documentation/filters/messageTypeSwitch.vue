<template>
  <el-dialog :visible.sync="dialogMsvisible" :before-close="initDialog" title="添加规则节点" top="11vh" >
...
export default {
  ...
  methods: {
    surechange() {
      this.$emit('onUpdateMessageTypeSwitData', this.ruleForm.name)
      this.$emit('onDialogMSvisibleChange', false) // 关闭对话框
    },
    initDialog() {
      this.$emit('onDialogMSvisibleChange', false) // 关闭对话框
