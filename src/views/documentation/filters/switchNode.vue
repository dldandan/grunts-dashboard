<template>
  <el-dialog :visible.sync="dialogSvisible" :before-close="initDialog" title="添加规则节点" top="2vh" >
    <el-form :model="ruleForm" :rules="rules" label-position="top" ref="ruleForm" label-width="180px" style="padding:20px;">
      ...
      <el-form-item label="Switch" prop="">
        　<span>function Switch(msg, metadata, msgType) { </span><br>
          ...
          <span>}</span><br>
          <el-button type="primary" @click="innerVisible = true">TEST SWITCH FUNCTION</el-button>
      </el-form-item>
      ...
      <el-row style="margin-top:15px;">
        <el-col :span="12">
          <el-card style="height:400px;">
            <div style="float:right;color:#00acc1;padding:4px;background:rgba(220, 220, 220, 0.35);border-radius: 5px;">SWITCH</div>
            <el-form label-width="110px" style="padding:20px;">
              <el-form-item label="" prop="">
                <span style="margin-left:-25px;">function Switch(msg, metadata, msgType) { </span><br>
...
export default {
  ...
  data() {
    return {
      ...
      innerVisible: false,
      item: {
        content: 'function nextRelation(metadata, msg) {return ["one","nine"];} if(msgType === "POST_TELEMETRY_REQUEST") {return ["two"];}return nextRelation(metadata, msg);',
  ...
  methods: {
    surechange() {
      this.$emit('onUpdateSwitchData', this.ruleForm.name)
      this.$emit('onDialogSvisibleChange', false) // 关闭对话框
    },
    initDialog() {
      this.$emit('onDialogSvisibleChange', false) // 关闭对话框
