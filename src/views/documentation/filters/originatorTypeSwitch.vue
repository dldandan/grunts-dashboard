<template>
  <el-dialog :visible.sync="dialogOsvisible" :before-close="initDialog" title="添加规则节点" top="11vh">
...
export default {
  ...
  methods: {
    surechange() {
      this.$emit('onUpdateOriginatorSwitchData', this.ruleForm.name)
      this.$emit('onDialogOSvisibleChange', false) // 关闭对话框
    },
    initDialog() {
      this.$emit('onDialogOSvisibleChange', false) // 关闭对话框
