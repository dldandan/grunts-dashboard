<template>
  <el-dialog :visible.sync="dialogTavisible" :before-close="initDialog" title="添加规则节点" top="11vh" >
...
export default {
  ...
  methods: {
    surechange() {
      this.$emit('onUpdateTenantData', this.ruleForm.name)
      this.$emit('onDialogTAvisibleChange', false)
    },
    initDialog() {
      this.$emit('onDialogTAvisibleChange', false) // 关闭对话框
