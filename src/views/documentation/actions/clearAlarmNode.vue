<template>
  <el-dialog :visible.sync="dialogClanvisible" :before-close="initDialog" title="添加规则节点" top="11vh" >
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        alarmType: [
          { required: true, message: 'Alarm type is required.', trigger: 'blur' }
        ]
      },
      messrules: {
        messtype: [
  ...
  methods: {
    surechange() {
      this.$emit('onUpdateClearAlarmData', this.ruleForm.name)
      this.$emit('onDialogCLANvisibleChange', false) // 关闭对话框
    },
    initDialog() {
      this.$emit('onDialogCLANvisibleChange', false) // 关闭对话框
