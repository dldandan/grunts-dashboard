<template>
  <el-dialog :visible.sync="dialogAtcvisible" :before-close="initDialog" title="添加规则节点" top="10vh">
    <el-form :model="ruleForm" :rules="rules" label-position="top" ref="ruleForm" label-width="120px" style="padding:30px;">
      ...
      <el-form-item label="" prop="">
        <el-checkbox label=" Create new customer if not exists" name="type" v-model="ruleForm.delivery"></el-checkbox><br>
      </el-form-item>
...
export default {
  ...
  methods: {
    surechange() {
      this.$emit('onUpdateAssignToCustomData', this.ruleForm.name)
      this.$emit('onDialogATCvisibleChange', false) // 关闭对话框
    },
    initDialog() {
      this.$emit('onDialogATCvisibleChange', false) // 关闭对话框
