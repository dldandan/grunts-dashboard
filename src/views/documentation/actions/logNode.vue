<template>
  <el-dialog :visible.sync="dialogLnvisible" :before-close="initDialog" title="添加规则节点" top="11vh" >
    <el-form :model="ruleForm" :rules="rules" label-position="top" ref="ruleForm" label-width="180px" style="padding:20px;height:450px;overflow:auto;">
      ...
      <el-form-item label="To string" prop="">
        　<span>function Tostring(msg, metadata, msgType) { </span><br>
          ...
          <span>}</span><br>
          <el-button type="primary" @click="innerVisible = true">TEST To STRING FUNCTION</el-button>
      </el-form-item>
      ...
      <el-row style="margin-top:15px;">
        <el-col :span="12">
          <el-card style="height:400px;">
            <div style="float:right;color:#00acc1;padding:4px;background:rgba(220, 220, 220, 0.35);border-radius: 5px;">To STRING</div>
            <el-form label-width="110px" style="padding:20px;">
              <el-form-item label="" prop="">
                <span style="margin-left:-25px;">function Tostring(msg, metadata, msgType) { </span><br>
...
export default {
  ...
  data() {
    return {
      ...
      innerVisible: false,
      item: {
        content: 'return "Incoming message:\n" + JSON.stringify(msg) + "\nIncoming metadata:\n" + JSON.stringify(metadata);',
  ...
  methods: {
    surechange() {
      this.$emit('onUpdateLogData', this.ruleForm.name)
      this.$emit('onDialogLNvisibleChange', false) // 关闭对话框
    },
    initDialog() {
      this.$emit('onDialogLNvisibleChange', false) // 关闭对话框
