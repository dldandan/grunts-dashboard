<template>
  <el-dialog :visible.sync="dialogCranvisible" :before-close="initDialog" title="添加规则节点" top="11vh" >
    <el-form :model="ruleForm" :rules="rules" label-position="top" ref="ruleForm" label-width="180px" style="padding:20px;height:450px;overflow:auto;">
      ...
      <el-form-item label="Alarm details builder" prop="">
        　<span>function Details(msg, metadata, msgType) { </span><br>
          ...
          <span>}</span><br>
          <el-button type="primary" @click="innerVisible = true">TEST DETAILS FUNCTION</el-button>
      </el-form-item>
      <el-form-item label="" prop="">
        <el-checkbox label=" Use message alarm data" name="type" v-model="ruleForm.delivery"></el-checkbox><br>
      </el-form-item>
      <el-form-item label="" prop="" v-if="ruleForm.delivery === false">
        ...
        <el-row>
          <el-col :span="12">
            <el-input v-model="ruleForm.alarmType" style="width:90%;"></el-input>
          </el-col>
          <el-col :span="12">
            <el-select v-model="ruleForm.alarmSeverity" placeholder="请选择" style="width:90%;">
                <el-option
                v-for="item in allSeverity"
                :key="item.value"
                :label="item.label"
                :value="item.value">
                </el-option>
      ...
      <el-form-item label="" prop="" v-if="ruleForm.delivery === false">
        <el-checkbox label=" Propagate" name="type" v-model="ruleForm.propagate"></el-checkbox><br>
      </el-form-item>
      ...
      <el-row style="margin-top:15px;">
        <el-col :span="12">
          <el-card style="height:400px;">
            <div style="float:right;color:#00acc1;padding:4px;background:rgba(220, 220, 220, 0.35);border-radius: 5px;">DETAILS</div>
            <el-form label-width="110px" style="padding:20px;">
              <el-form-item label="" prop="">
                <span style="margin-left:-25px;">function Details(msg, metadata, msgType) { </span><br>
...
export default {
  ...
  data() {
    return {
      ...
      innerVisible: false,
      item: {
        content: 'var details = {};if (metadata.prevAlarmDetails) {details = JSON.parse(metadata.prevAlarmDetails);return details;}',
  ...
  methods: {
    surechange() {
      this.$emit('onUpdateCreateAlarmData', this.ruleForm.name)
      this.$emit('onDialogCANvisibleChange', false) // 关闭对话框
    },
    initDialog() {
      this.$emit('onDialogCANvisibleChange', false) // 关闭对话框
